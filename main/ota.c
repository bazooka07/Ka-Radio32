...
void otaTask(void *pvParams) {
	...
	esp_err_t err = esp_http_client_open(client, 0); // 0 : only reading
	if(err != ESP_OK) {
		ESP_LOGE(TAG,
	...
	if(content_length < 0) {
		ESP_LOGI(TAG, "HTTP error %d", esp_http_client_get_status_code(client));
		http_cleanup(client);
		task_fatal_error();
		...
		if(http_status != 200) {
			ESP_LOGI(TAG, "HTTP error %d", esp_http_client_get_status_code(client));
			http_cleanup(client);
			wsUpgrade("http_error", 0, http_status);
			task_fatal_error();
		}
	}
	ESP_LOGI(TAG, "Content-length of firmware : %u", content_length);
