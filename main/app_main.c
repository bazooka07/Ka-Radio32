...
static void start_wifi() {
	...
	if(g_device->current_ap == APMODE) {
		...
		} else if(strlen(CONFIG_WIFI_SSID) > 0 && strcmp(CONFIG_WIFI_SSID, "myssid") != 0) {
	...
	while(true) {
		...
		if (g_device->current_ap == APMODE) {
			...
			ESP_LOGE(TAG, "\n\
=============================================\n\
  This device is in ACCES POINT mode.\n\
  Set ssid for your wifi with \"KaRadio32\".\n\
  No password.\n\
  Load page at \"192.168.4.1\" with your web-browser\n\
  And go to \"Setting\" tab\n\
  May take a while for the first connection.\n\
  Be patient....\n\
=============================================\
");
